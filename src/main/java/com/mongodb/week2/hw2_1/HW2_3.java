package com.mongodb.week2.hw2_1;
 ...
 * for each student from the dataset in the handout. Since each document is one grade, it should remove one document per student.
...
public class HW2_3 {
    ...
    public static void main(String[] args) {
        ...
        Document matches = new Document("$match", new Document("type", "homework"));    //find type='homework'
        //group by student_id and make min aggregation function by 'score' field
        Document groupBy = new Document("$group", new Document("_id", "$student_id").append("min", new Document("$min", "$score")));
        Document sort = new Document("$sort", new Document("_id", 1)); //sort by _id=_student_id
        AggregateIterable<Document> aggregate = grades.aggregate(Arrays.asList(matches, groupBy, sort));

        for (Document document : aggregate) {
            grades.deleteOne(and(eq("student_id", document.get("_id")), eq("score", document.get("min"))));//delete by BSON
